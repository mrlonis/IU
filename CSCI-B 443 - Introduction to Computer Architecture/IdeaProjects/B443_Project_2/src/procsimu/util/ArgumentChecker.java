/***************************************************************
 ...
 Classe   :  ArgumentChecker
 Heritage :  -

 Description :   Methodes pour manipuler des arguments de ligne
 de commandes

 Date de creation     :  02/12/97
 Liste des modifications :
 Date            Auteur        Modifications
 --------------------------------------------
 02/12/97        Kevin Swan    Creation
 29/01/00        V. Oberle     Used in "Simulation of Processor Architecture"
...
package procsimu.util;

import java.util.Vector;

/**
 * This class provides several static methods for checking, manipulating,
 * and accessing command line arguments.  It allows you to check if a
 * valid argument set was given to your program with one simple call to
 * a static method.  It also provides a single method to check for the
 * presence of a switch, or for retrieving the value associated with
 * a switch.
 * <p>
 * <P>The main functionality of this class is based around the concept
 * of a description String.  A description String is a simple, custom
 * regular expression which describes a legal command line for your
 * program.  A switch is indicated with a '-'.  A value is any String
 * that does not start with a '-', '[', ']', or '|', or the usual
 * reserved characters for your shell or command line interpreter.
 * <p>
 * <P>Switches and arguments which are optional are enclosed in square
 * brackets.  When a choice exists, the '|' character indicates an
 * <CODE>OR</CODE> situation.  An example of a description String would
 * be:<BR>
 * <CODE>
 * &quot;-version | -help | -s server [ -p port | -i interval ]&quot;
 * </CODE>
 * <p>
 * <P>The above description String means the following.  You could invoke
 * the program and give it ONLY the argument &quot;-version&quot;.  Or,
 * you could invoke it and give it ONLY the argument &quot;-help&quot;.
 * Or, you could invoke it and give it a &quot;-s&quot;, then a server
 * name.  If you do this, you can include other parameters after the
 * &quot;-s server&quot; part, if you want.  You could give it a
 * &quot;-p&quot; and some string representing a port number, or you
 * could give it a &quot;-i&quot; and some String representing an interval.
 * You could NOT give it anything after the &quot;-version&quot; String,
 * or the &quot;-help&quot; String, if you used either of those.  Note that
 * all tokens in the description MUST be seperated by spaces.
 * <p>
 ...
 * </PRE>
 * <p>
 * <P>This version has undergone a complete rewrite.  If you find any bugs,
 * please let me know at 013639s@dragon.acadiau.ca.
 *
 * @author Kevin Swan
...
public class ArgumentChecker {
    ...
    /**
     * Don't let anyone instantiate this class.  Its only used for the
     * static methods.
     */
    ...
    /**
     * This method checks if the given arguments legally satisfy the given
     * description.  The description in this version is very simple.  It is
     * merely a String describing legal command line argument sets in a
     * generic format.  If a parameter in the description starts with a '-',
     * then the entire item must be matched exactly in the arguments.  If a
     * parameter in the description starts with a '[', then it is an optional
     * argument.  An example would be to pass the array {"-s" "schedule.dat"}
     * and the description "[ -s datafile ] [ -d dayOfWeek ]".  This should
     * return true.
     * <p>
     * <P>Nesting is supported in versions 1.4 and above.  This means that
     * a description String such as "[ -s datafile [ -o outputfile ] ] -d day"
     * would be perfectly legal.  The level of nesting is restricted only by
     * Java or the amount of memory in your machine.
     *
     * @param args        the command line arguments.
     * @param description a simple regular expression describing legal
     *                    arguments.
     * @return true of the given arguments satisfy the description, false
     * otherwise.
     */
    ...
    private static boolean isValid(String description,
        ...
        return ArgumentChecker.isValid(new KSStringTokenizer(argString),
                description,
                permitExtraArgs);
    }
     ...
     */
    private static boolean isValid(KSStringTokenizer argTokenizer,
                                   String description,
                                   boolean permitExtraArgs) {
        ...
        if (ArgumentChecker.numSections(description, '|') > 1) {
            ...
            if (!argTokenizer.hasMoreTokens()) {
                for (int i = 0; i < subBlocks.size(); i++)
                    if (minNumber((String) subBlocks.elementAt(i)) == 0)
                        return true;
         ...
         */
        if (argTokenizer.countTokens() < min)
            return false;

        /*
         * But if we're permitting extra args, we may have more than max.
         */
        if (argTokenizer.countTokens() > max)
            if (!permitExtraArgs)
                return false;
        ...
        while (true) {
            ...
            while (true) {
                 ...
                 */
                if (ArgumentChecker.match(currArgToken, tok))
                    break;
                else
                    /*
                ...
                if (tok.equals("[")) {
                    ...
                    if (matchMade) {
                         ...
                         */
                        if (!ArgumentChecker.isValid(argTokenizer, optionalDescription, true))
                            return false;
    ...
    /**
     * Returns the minimum number of allowed arguments based on the
     * given description.
     *
     * @param description the String describing the allowable argument
     *                    string as described above in the comment for isValid().
     * @return the minimum number of arguments which may be supplied
     * based on the given description.  Returns -1 if an error
     * occurs.
     */
    ...
    /**
     * Returns the maximum number of allowed arguments based on the
     * given description.
     *
     * @param description the String describing the allowable argument
     *                    string as described above in the comment for isValid().
     * @return the maximum number of arguments which may be supplied
     * based on the given description.  Returns -1 if an error
     * occurs.
     */
    ...
    /**
     * This method is used to get the value associated with a switch.
     * It supports only very simply value association; one switch to
     * one value.  It is used as follows:
     * <PRE>
     ...
     * @return the value associated with the named switch, if the named
     * switch exists in the args array.  If the switch is not
    ...
    /**
     * This method is used to determine if a particular argument is
     * found in the argument array.  It is intended to detect whether
     * or not a switch is in the arg array, but it doesn't check for
     * a '-'.  It will return true if switchID is exactly equal to
     * any String in the argument array.
     *
    ...
    /**
     * This routine determines whether we should consider the given arg
     * to match the description, tok, if it appeared on the command line.
     *
    ...
    /**
     * This routine is used to find all the possible FIRST tokens in an
     * optional block for the purposes of deciding whether we need to
     * make a recursive isValid() call on this block.  It returns a Vector
     * containing Strings, any one of which could legally appear as the
     * first arg to satisfy this block.
     * <p>
     * <P>EXAMPLE:  If it is called given the String <BR>
     * <CODE>&quot;[ -s server ] [ -p port ] name&quot;</CODE><BR>
     * then the result would be a Vector containing the Strings
     * &quot;-s&quot; &quot;-p&quot; and &quot;name&quot;.
     *
    ...
    private static Vector firstTokens(String description) {
        ...
        while (tokenizer.hasMoreTokens()) {
            ...
            if (tok.equals("[")) {
                 ...
                 */
                for (int i = 0; i < subFirsts.size(); i++)
    ...
    /**
     * This method is used to determine how many distinct sections
     * there are in the given description, using the given character
     * as the seperator.  It is intended to be used to count how
     * many OR blocks there are.  The sep variable will probably be
     * the '|' character.  It keeps track of depth as well, so if it
     * is given a description such as<BR>
     * <CODE>
     * &quot;-s server | [ -p port | -default ]&quot;<BR>
     * </CODE>
     * it would return 2, not 3, even though there are 2 seperators.
     *
     * @param description the description String to evaluate
     * @param sep         the character to use as the seperator token.
     * @return the number of sections in the description, seperated
     * by the given sep token.  Returns 0 if an error occurs.
     */
    ...
    /**
     * This method is used to seperate out the various blocks based on the
     * seperator token (usually '|', for OR).  It returns a Vector of all the
     * distinct blocks possible from the given description String.  For
     * example, if the description String given were<BR>
     * <CODE>
     * &quot;-s server | [ -p port | -default ]&quot;<BR>
     * </CODE>
     * this method would return a Vector containing the following 2
     * Strings:<BR>
     * <UL>
     * <LI>&quot;-s server&quot;</LI>
     * <LI>&quot;[ -p | -default ]&quot;</LI>
     ...
     * @param sep         the character to use as the seperator token, usually
     *                    '|' for OR.
     * @return a Vector containing all the different '|' seperated top-level
     * Strings parsed out of description.
     */
