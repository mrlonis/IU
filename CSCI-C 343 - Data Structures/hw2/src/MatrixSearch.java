 ...
 * 
 * In the comment at the top of the program, write a brief explanation to justify your running time.
 * 
 * Answer:
 * 
 * The runtime is O(N). This is because our while loop runs at most N times. The contents inside
 * the while loop all have a RunTime of O(1). 1 * N = N so O(N). The reason this is O(N) and not
 * O(N^2) is because although yes i am manipulating two variables to move throughout the array, i am
 * not using a nested for loop. I also am not analyzing every single cell in the array to search for the
 * value i am looking for. Based on my algorithm, since i start in the top right cell, and the value im looking
 * for is in the bottom left cell, my movement throughout the matrix would appear as a diagonal rather than
 * a scan through the whole matrix.
 * 
 * @author mrlonis
 */
