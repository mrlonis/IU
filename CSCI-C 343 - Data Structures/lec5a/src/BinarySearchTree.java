 ...
 * 
 * (1) Add javadoc style comments to all methods. (2) Implement
 * BinarySearchTree.contains() so that it runs in O(h) time, where h is the
 * height of the tree. (3) Modify Tree and BinarySearchTree so that they are
 * generic for any Comparable type object. (4) [challenge] Try to implement a
 * sensible BinarySearchTree.toString() method. Recall that an in order
 * traversal of a BST yields a sorted sequence. (5) Test thoroughly in main().
 * Be sure to include tests on non-integer data.
 * 
 * @author green1
...
public class BinarySearchTree<K extends Comparable<K>> implements Tree<K> {
	...
	class Node {
		 ...
		 * 
		 * @param key
		 *            The value to be put into the data field for the Node being
		 *            created.
		 */
		Node(K key) {
			this(key, null, null);
		 ...
		 * 
		 * @param data
		 *            The data to be placed in the Node being created.
		 * @param left
		 *            The left Node pointer for this Node.
		 * @param right
		 *            The right Node pointer for this Node.
		 */
		 ...
		 * 
		 * @return <code>true</code> iff the left and right fields for a Node
		 *         are <code>null</code>.
		 */
		boolean isLeaf() {
	 ...
	 * 
	 * @param key
	 *            The data for the Node that is to be placed in the BST.
	 */
	...
	/**
	 * Searches for the correct spot to place a new Node with the data key.
	 * Updates every pointer in the BST, but the only noticable change will be
	 * that a new Node is inserted.
	 * 
	 * @param key
	 *            The data for the Node to be inserted into the BST.
	 * @param p
	 *            The pointer to the current Node. This is the Node that will
	 *            have either its left or right pointers updated.
	 ...
	 * 
	 * @return <code>true</code> iff the BST contains a Node with
	 *         <code>key</code> as its data member.
	 */
	 ...
	 * 
	 * @param key
	 *            The data to check each Node for.
	 * @param p
	 *            Pointer to the Node this method is checking in.
	 ...
	 * 
	 * @param string
	 *            The StringBuilder receieved from toString to continue building
	 *            onto.
	 * @param node
	 *            The cirrent Node in the BST.
	...
	public static void main(String... args) {
		...
		String[] b = new String[] { "Matthew", "Nova", "Chris", "Dylan", "Clements", "Audretsch", "Richardson",
				"Lonis" };
...
interface Tree<K extends Comparable<K>> {
	 ...
	 * 
	 * @param key
	 *            The item to search for in a Tree.
