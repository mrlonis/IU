/**
 * StringMatch class with static utilities to find patterns in strings. This
 * class features the Naive string matching algorithm, the KMP matching
 * algorithm, and the Boyer-Moore string matching algorithm.
 * 
...
public class StringMatch {
	 ...
	 * @return The Result of running the naive algorithm to match pattern in
	 *         text.
	 */
	 ...
	 * @return The number of character comparisons performed during the process
	 *         of building flink.
	 */
	 ...
	 * @param delta1
	 *            The array to be filled that corresponds to the Boyer Moore
	 *            delta1.
	 */
	 ...
	 * @return The Result of running the simplified Boyer-Moore algorithm using
	 *         the delta1 from the pre-processing phase.
	 */
			...
			while (j >= 0 && i < n) {
				...
				if (d == c) {
					...
					if ((int) c >= Constants.SIGMA_SIZE || (int) c < 0) {
						...
						i += Math.max(delta1[(int) c] - partialMatch, 1);
					}
					finger = i;
					partialMatch = 0;
				}
