...
class BTree {
	...
	/**
	 * A structure to pinpoint the location of a key in the tree. Used as the
	 * return type of the search() method.
	 */
	class Location {
		Node p;
		int i;
	...
	class Node {
		...
		/**
		 * Returns the index of key in keys, if it exists. Otherwise,the index
		 * of where key can be inserted into keys to preserve the sorted order
		 * is returned. Note: the return value could be keys.size(), so need to
		 * check for that before using the index on a get(). This is a helper
		 * for Node.insert() and Node.search().
		 */
		...
		/**
		 * Returns the location of the given key in the tree rooted at this
		 * node. If the key is not found, then null is returned. This is a
		 * helper for BTree.search().
		 */
		...
		/**
		 * Pre-conditions: This node is not full and the i-th child of this node
		 * is full. Consequently, this node is not a leaf.
		 * 
		 * The middle key from the i-th child is propagated up into this node
		 * and the remaining keys in the i-th child are split as evenly as
		 * possible amongst left and right siblings. In the case that b is even,
		 * there are two middle keys and we choose the one on the left;
		 * consequently the right sibling will have one more key than the left.
		 * 
		 * Post-conditions: This node will have one more key (and one more
		 * child). The i-th child of this node will be half full, as will its
		 * right sibling.
		 */
		void splitChild(int i) {
			 ...
			 * Propagate the middle key into this node, along with a pointer to
			 * the right sibling.
			 */
		...
		void insert(int key) {
			 ...
			 * TODO #3
			 * 
			 * This code assumes that key does not already exist in the tree.
			 * Make the appropriate changes to ensure that an attempt to insert
			 * a duplicate keys does nothing.
			 */
	...
	/**
	 * Insert key into this tree. If the key already exists, then nothing is
	 * done.
	 */
	void insert(int key) {
		if (root.isFull()) {
			 ...
			 * child needs to split later. Do this by creating a new root with
			 * no keys and the old root as its only child, and then calling
			 * splitChild() to split the full child.
			 */
