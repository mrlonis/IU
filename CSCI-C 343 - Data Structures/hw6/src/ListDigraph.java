...
/**
 * ListDigraph is a class that implements a directed graph using a Map of
 * adjacency lists.
 * 
 * A digraph may have self-loops but no parallel edges. We assume that vertices
 * are labeled by the integers 0, 1, ..., n - 1. The edges are labeled with a
 * positive weight.
 * 
 * Give the big-Oh cost for each of the following operations in terms of n
 * (the number of vertices): in O(?) out O(?) addEdge O(?) weight O(?)
 * numVertices O(?) numEdges O(?) twoHopsAway O(?)
 * 
