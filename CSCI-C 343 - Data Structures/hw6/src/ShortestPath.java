...
public class ShortestPath {
	...
	private static int shortestPathHelper(int s, int t, Digraph gr, Map<Integer, Integer> cache) {
		...
		if (s == t)
			...
			for (int x : gr.in(t))
				ans = Math.min(ans, add(shortestPathHelper(s, x, gr, cache), gr.weight(x, t)));
		}
		cache.put(t, ans);
	...
	/**
	 * Returns x + y, where x and y are positive integers. In the case of
	 * overflow, Integer.MAX_VALUE is returned.
	 */
