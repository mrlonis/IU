...
public class Result {
	...
	/**
	 * The direction of the neighboring entry in the cache from which this entry
	 * is extended. (See definition of Direction below.)
	 */
...
/**
 * We use this enumeration to represent the direction of the parent result along
 * the optimal path. If the largest score comes from an M operation (i.e., a
 * match or a mismatch), then the direction is DIAGONAL. If the largest score
 * comes from an I operation (i.e., a gap is inserted into strand x), then the
 * direction to the parent is UP. If the largest score comes from a D operation
 * (i.e., a gap is inserted into strand y), then the direction to the parent is
 * LEFT.
 */
