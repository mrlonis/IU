 ...
 * 
 * The Sequence Aligner determines the bets possible alignment between two
 * strings by filling in an array of Results (size: n x m) based on a given
 * Judge.
 * 
...
public class SequenceAligner {
	...
	/**
	 * Generates a pair of random DNA strands, where x is of length n and y has
	 * some length between n/2 and 3n/2, and aligns them using the default
	 * judge.
	 */
	 ...
	 * the result of solving the alignment problem for the first i characters in
	 * x and the first j characters in y.
	 * 
	 * Your algorithm must run in O(n * m) time, where n is the length of x and
	 * m is the length of y.
	 * 
	 * Ordering convention: So that your code will identify the same alignment
	 * as is expected in Testing, we establish the following preferred order of
	 * operations: M (diag), I (left), D (up). This only applies when you are
	 * picking the operation with the biggest payoff and two or more operations
	 * have the same max score.
	 */
	...
	/**
	 * Returns the result of solving the alignment problem for the first i
	 ...
	 * with the Result in the lower right corner of the cache. Run
	 * Result.markPath() on each Result along the path. The GUI will highlight
	 * all such marked cells when you check 'Show path'. As you're tracing back
	 * along the path, build the aligned strings in alignedX and alignedY (using
