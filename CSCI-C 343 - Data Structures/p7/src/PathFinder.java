...
public class PathFinder {
	 ...
	 * connected wires on the grid. If it is not possible to connect the end
	 * points of a wire, then there should be no association for the wire id# in
	 * the result.
	 */
	...
	/**
	 * This method processes a given wire, finding the optimal path to connect
	 * the wire. If the path cannot be found, a null value is returned if the
	 * path wasn't able to be completed, or a pair is returned that contains a
	 * null path but a string containing the wireID of the wire preventing this
	 * wire from connecting.
	 * 
	 * @param wire
	 *            The wire to connect.
	 * @param visited
	 *            The visited matrix corresponding to coordinates that can be
	 *            added to the path without interfering with any obstacles or
	 *            other wires.
	 * @param chip
	 *            The chip this wire exists inside of.
	 * @param wildCard
	 *            A wild card string. If this is anything other than "Ignore"
	 *            then the algorithm will use this as a reference of a direction
	 *            to avoid using on its first move.
	 * @return A pair containing the found path, if there is one, and a String
	 *         referencing either the first direction the algorithm made or the
	 *         wireID of the wire preventing this wire from being able to
	 *         connect. In special circumstances, such as the wire being unable
	 *         to connect due to obstacles, null is returned.
	 */
	...
	/**
	 * This method sorts the wires into a PriorityQueue based on the separation
	 * distance of the wires. The wires with the smallest separation distance
	 * will be placed at the top of the queue while the longer separations will
	...
	/**
	 * Constructs the initial visited array placing true into every coordinate
	 * that is either inside an obstacle or an end point of a wire.
	 * 
	 * @param chip
	 *            The chip this visited array will reference from.
	 * @return A boolean matrix of true and false where true is wherever an
	 *         obstacle is or wherever an end point of a wire is.
	 */
