 ...
 */
let bezier : GLint = 0;
let catmull_rom : GLint = 1;
let b_spline : GLint = 2;
let no_spline : GLint = -1;

let proximity_result_noop : Int = -1;
let proximity_result_insert : Int = 0;
let proximity_result_vertice_edit : Int = 1;
let proximity_result_line_edit : Int = 2;
...
/*
 * t-value will be determined in GPU by mult Point x or y value with 0.02
 */
var blank_data : [GLfloat] = [
    0.0, 0.0,    // Point 0
    1.0, 1.0,    // Point 1
    2.0, 2.0,    // Point 2
    3.0, 3.0,    // Point 3
...
class MyOpenGLESKitViewController: GLKViewController {
    @IBOutlet var catmull_rom_button: UIButton!;
    @IBOutlet var b_spline_button: UIButton!;
    @IBOutlet var bezier_button: UIButton!;
    @IBOutlet var reset_button: UIButton!;
    @IBOutlet var first_derivative_button: UIButton!;
    @IBOutlet var second_derivative_button: UIButton!;
    ...
    // Data to store for proximity testing
    var proximity_result: [Int] = [-1, 0, 0, 0, 0];
    
    // Variables for splines
    var spline_type : GLint = 0; // Used in shader
    var draw_spline : GLint = 0; // Used in shader
    var controller_spline_type : GLint = 0; // Used in this view controller to determine i inc
    
    // Control points for splines
    var p0 : GLint = 0; // Used in shader
    var p1 : GLint = 0; // Used in shader
    var p2 : GLint = 0; // Used in shader
    var p3 : GLint = 0; // Used in shader
    
    var t_inc : GLint = 0; // Used in shader
    
    var draw_first_derivative : Int = 0; // Used in this view controller
    var draw_second_derivative : Int = 0; // Used in this view controller
    ...
    override func glkView(_ view: GLKView, drawIn rect: CGRect) {
        ...
        // Set t inc value to 0.02
        glUniform1f(self.t_inc, GLfloat(0.02));
        ...
        /*
         * This handles drawing the lines
         *
         * Note: This does not draw the highlighted lines for editing
         */
        if (lNumberOfVertices >= 2) {
            // Set color to: green (1)
            glUniform4f(self.myColorUniform, gColorData[1][0], gColorData[1][1], gColorData[1][2], gColorData[1][3]);
            
            if (self.proximity_result[0] == proximity_result_line_edit) {
        ...
        /*
         * This if statement draw the vertices
         *
         * Note: This does not draw the vertices that are selected for editing
         */
        if (lNumberOfVertices >= 1) {
            // Set color to: red (0)
            glUniform4f(self.myColorUniform, gColorData[0][0], gColorData[0][1], gColorData[0][2], gColorData[0][3]);
            
            if (self.proximity_result[0] == proximity_result_vertice_edit) {
                if (self.proximity_result[1] == 0) {
        ...
        /*
         * This if statement draws the splines if there are more
         * than 4 vertices
         */
        if (lNumberOfVertices >= 4) {
            glDisableVertexAttribArray(self.myPosAttrib);
            glEnableVertexAttribArray(self.myPosAttrib);
            
            glVertexAttribPointer(self.myPosAttrib, 2, GLenum(GL_FLOAT), GLboolean(GLenum(GL_FALSE)), 0, &blank_data);
            
            var i = 0;
            
            glUniform1i(self.draw_spline, GLint(1));
            
            // Set color to: white (3)
            glUniform4f(self.myColorUniform, gColorData[3][0], gColorData[3][1], gColorData[3][2], gColorData[3][3]);
            
            while (i+8 <= gVertexData.count) {
                glUniform4f(self.p0, gVertexData[i],   gVertexData[i+1], 0, 1);
                glUniform4f(self.p1, gVertexData[i+2], gVertexData[i+3], 0, 1);
                glUniform4f(self.p2, gVertexData[i+4], gVertexData[i+5], 0, 1);
                glUniform4f(self.p3, gVertexData[i+6], gVertexData[i+7], 0, 1);
                    
                glDrawArrays(GLenum(GL_LINE_STRIP), 0, GLsizei(blank_data.count / 2));
                
                if (self.controller_spline_type == bezier) {
                    i = i + 6;
                } else if (self.controller_spline_type == catmull_rom || self.controller_spline_type == b_spline) {
                    i = i + 2;
                }
            }
            
            glUniform1i(self.draw_spline, GLint(0));
        }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let firstTouch = touches.first  {
            ...
            if (self.proximity_result[0] == proximity_result_insert) {
                E_MODE_INSERT(xCoord: self.myTouchXbegin, yCoord: self.myTouchYbegin);
            } else if (self.proximity_result[0] == proximity_result_vertice_edit || self.proximity_result[0] == proximity_result_line_edit) {
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let firstTouch = touches.first  {
            ...
            /*
             * Call EDIT regardless if editing since if proximity_result[0] == proximity_result_insert, nothing will happen
             */
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let firstTouch = touches.first  {
            ...
            self.proximity_result = [proximity_result_noop, 0, 0, 0, 0];
     ...
     * E_MODE_INSERT
     *
     * This method handles the insertion of a new vertex
     */
    func E_MODE_INSERT(xCoord: GLfloat, yCoord: GLfloat) {
        ...
        /**
         * Remove all verticies if verticies exceed 32
         */
        if (gVertexData.count > 64) {
    ...
    func E_MODE_PROXIMITY(xCoord: GLfloat, yCoord: GLfloat) -> [Int] {
        var ans = [proximity_result_insert, 0, 0, 0, 0];
        ...
        while (i < gVertexData.count) {
            ...
            if (abs(xCoord - x1) <= point_threshold && abs(yCoord - y1) <= point_threshold) {
                ans = [proximity_result_vertice_edit, i, i+1, 0, 0];
                return ans;
            }
        ...
        while (i <= gVertexData.count - 4) {
            ...
            if (d <= line_threshold) {
                ans = [proximity_result_line_edit, i, i+1, i+2, i+3];
                return ans;
            }
     ...
     * minimum_distance
     *
     * This function returns the minimum distance between a point and a line.
     * This will help us to compare the result of this with line_threshold
     */
     ...
     * E_MODE_EDIT
     *
     * This function is used for editing vertices or lines
     */
    func E_MODE_EDIT(xCoord: GLfloat, yCoord: GLfloat, data: [Int], xPast: GLfloat, yPast: GLfloat) {
         ...
         */
        if (data[0] == proximity_result_vertice_edit) {
            gVertexData[data[1]] = xCoord;
            gVertexData[data[2]] = yCoord;
        } else if (data[0] == proximity_result_line_edit) {
     ...
     * catmull_rom_action
     *
     * This function sets spline_type to 1 as well as sets the button
     * background color to green and the rest of the button background
     * colors to cyan.
     */
    @IBAction func catmull_rom_action(_ sender: UIButton) {
        glUniform1i(self.spline_type, catmull_rom);
        self.controller_spline_type = catmull_rom;
        catmull_rom_button.backgroundColor = UIColor.green;
        b_spline_button.backgroundColor = UIColor.cyan;
        bezier_button.backgroundColor = UIColor.cyan;
    }
    
    /**
     * b_spline_action
     *
     * This function sets spline_type to 2 as well as sets the button
     * background color to green and the rest of the button background
     * colors to cyan.
     */
    @IBAction func b_spline_action(_ sender: UIButton) {
        glUniform1i(self.spline_type, b_spline);
        self.controller_spline_type = b_spline;
        catmull_rom_button.backgroundColor = UIColor.cyan;
        b_spline_button.backgroundColor = UIColor.green;
        bezier_button.backgroundColor = UIColor.cyan;
    }
     ...
     * bezier_action
     *
     * This function sets spline_type to 0 as well as sets the button
     * background color to green and the rest of the button background
     * colors to cyan.
     */
    @IBAction func bezier_action(_ sender: UIButton) {
        glUniform1i(self.spline_type, bezier);
        self.controller_spline_type = bezier;
        catmull_rom_button.backgroundColor = UIColor.cyan;
        b_spline_button.backgroundColor = UIColor.cyan;
        bezier_button.backgroundColor = UIColor.green;
    }
    
    /**
     * I added this for my own use because I found the reset at 32 vertices
     * (which I do have implemented) to be too high
     */
    ...
    /**
     * This manipulates the button however I don't actually have the derivative drawing working
     */
    @IBAction func first_derivative_action(_ sender: UIButton) {
        if (self.draw_first_derivative == 0) {
            self.draw_first_derivative = 1;
            self.first_derivative_button.backgroundColor = UIColor.green;
        } else if (self.draw_first_derivative == 1) {
            self.draw_first_derivative = 0;
            self.first_derivative_button.backgroundColor = UIColor.cyan;
        }
    }
     ...
     * This manipulates the button however I don't actually have the derivative drawing working
     */
    @IBAction func second_derivative_action(_ sender: UIButton) {
        if (self.draw_second_derivative == 0) {
            self.draw_second_derivative = 1;
            self.second_derivative_button.backgroundColor = UIColor.green;
        } else if (self.draw_second_derivative == 1) {
            self.draw_second_derivative = 0;
            self.second_derivative_button.backgroundColor = UIColor.cyan;
        }
    }
    
    // ------------------------------------------------------------------------
    // MARK: -  OpenGL ES 3 shader compilation, linking, binding
    // ------------------------------------------------------------------------
    ...
    func loadShaders() -> Bool {
        ...
        // Get location of spline type variable in shader:
        self.spline_type = glGetUniformLocation(myGLESProgram, "u_spline_type");
        self.draw_spline = glGetUniformLocation(myGLESProgram, "u_draw_spline");
        
        self.p0 = glGetUniformLocation(myGLESProgram, "u_p0");
        self.p1 = glGetUniformLocation(myGLESProgram, "u_p1");
        self.p2 = glGetUniformLocation(myGLESProgram, "u_p2");
        self.p3 = glGetUniformLocation(myGLESProgram, "u_p3");
        
        self.t_inc = glGetUniformLocation(myGLESProgram, "u_t_inc");
