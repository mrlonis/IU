...
var blank_data : [GLfloat] = Array(repeating: 0, count: 40);
...
class MyOpenGLESKitViewController: GLKViewController {
    ...
    override func glkView(_ view: GLKView, drawIn rect: CGRect) {
        ...
        if (lNumberOfVertices >= 2) {
            // what color to use for the line strip: 0 = green
        ...
        if (lNumberOfVertices >= 4) {
            let number_of_splines : Int = Int(round(Double(lNumberOfVertices / 4)));
            let lmessage = "Number of Splines =  \(number_of_splines)";
            NSLog(lmessage);
            ...
            if (spline_type == 0) {
                ...
                let error_message = "spline_type > 2 - This is an Error!";
                NSLog(error_message);
            }
        }
