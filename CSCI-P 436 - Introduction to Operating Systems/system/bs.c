...
int bs_mkdev(int dev, int blocksize, int numblocks) {
    if (dev != 0) {
        printf("bs_mkdev(): Unsupported device: %d\n", dev);
        return SYSERR;
    }
    ...
    if ((dev0_blocks = getmem(dev0_numblocks * dev0_blocksize)) == (void *)SYSERR) {
        printf("bs_mkdev(): mkbsdev memgetfailed\n");
        return SYSERR;
    }
...
int bs_bread(int dev, int block, int offset, void *buf, int len) {
    ...
    if (dev != 0) {
        printf("bs_bread(): Unsupported device\n");
        return SYSERR;
    }
    
    if (offset >= dev0_blocksize) {
        printf("bs_bread(): Bad offset\n");
        return SYSERR;
    }
...
int bs_bwrite(int dev, int block, int offset, void *buf, int len) {
    ...
    if (dev != 0) {
        printf("bs_bwrite(): Unsupported device\n");
        return SYSERR;
    }
    
    if (offset >= dev0_blocksize) {
        printf("bs_bwrite(): Bad offset\n");
        return SYSERR;
    }
